...
import { NewWindowCopyEventName } from '@/constant'
import "./index.module.less";

const AutoCopy: FC = () => {
  useMount(() => {
    window.navigator.serviceWorker.controller?.postMessage({
      type: NewWindowCopyEventName.COPY_PAGE_READY
    });
    window.navigator.serviceWorker.addEventListener('message', async (e) => {
      if (e.data.type === NewWindowCopyEventName.COPY_CODE && e.data.data) {
        try {
          window.focus();
          if (document.hasFocus()) {
            await window.navigator.clipboard.writeText(e.data.data);
            window.navigator.serviceWorker.controller?.postMessage({
              type: NewWindowCopyEventName.COPY_CODE_FINISHED
            });
          } else {
            window.navigator.serviceWorker.controller?.postMessage({
              type: NewWindowCopyEventName.COPY_CODE_FAILED,
              data: `复制失败，请保持页面聚焦`
            });
          }
        } catch (e) {
